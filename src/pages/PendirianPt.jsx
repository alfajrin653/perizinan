...
const ptPackages = [
  {
    ...
    badge: 'STARTER',
    featured: false,
    features: ['Pengecekan & Pemesanan Nama PT', 'Akta Pendirian Notaris', 'SK Pengesahan Kemenkumham', 'NIB & Sertifikat Standar', 'NPWP & SKT Perusahaan'],
  ...
  },
  {
    ...
    features: ['Semua kelengkapan legalitas PT', 'Layanan Resepsionis Profesional', 'Bantuan Pembukaan Rekening Bank'],
...
export default function PembuatanPT() {
  return (
    <>
      <Helmet>
        <title>Jasa Pembuatan PT Cepat & Resmi - Duduk Manis</title>
        <meta name='description' content='Layanan jasa pembuatan PT resmi, proses kilat, biaya transparan, dan dibantu sampai tuntas. Mulai bangun legalitas perusahaan Anda bersama Duduk manis.' />
      </Helmet>
        ...
        <section className='py-24 bg-slate-50'>
          <div className='max-w-7xl mx-auto px-6 lg:px-8'>
            <div className='text-center max-w-3xl mx-auto mb-16'>
              <h2 className='text-3xl md:text-4xl font-bold mb-4 text-slate-900'>Pilih Paket Pendirian PT Anda</h2>
              <p className='text-slate-600'>Harga transparan tanpa biaya tersembunyi. Dapatkan bonus ekstra untuk mendukung kelancaran operasional bisnis Anda.</p>
            </div>
            <div className='grid md:grid-cols-2 gap-8 max-w-5xl mx-auto'>
              {ptPackages.map((pkg, index) => (
                <CompanyPackageCard key={index} name={pkg.title} price={pkg.price} badge={pkg.badge} featured={pkg.featured} features={pkg.features} bonuses={pkg.bonuses} />
              ))}
            </div>
          </div>
        </section>
