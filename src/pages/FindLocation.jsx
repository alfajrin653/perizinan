...
import { virtualOfficePackages as dataVO } from "../data/VirtualOffice"; 
...
const uniqueCities = [
  "Semua", 
  ...new Set(masterLocations.map(item => item.city).filter(Boolean))
];

// Ambil daftar layanan secara unik
const serviceTypes = ["Semua", "Virtual Office", "Meeting Room", "Coworking Space", "Podcast Studio"];
    ...
    <div className="min-h-screen bg-slate-50 pt-24 pb-16">
      ...
      <section className="px-4 max-w-7xl mx-auto mb-12 text-center">
        ...
        <p className="text-lg text-slate-600 max-w-2xl mx-auto mb-10">
          Dari SCBD hingga Mega Kuningan, jelajahi berbagai pilihan Virtual Office, Coworking Space, dan fasilitas bisnis lainnya.
        </p>
      ...
      <section className="px-4 max-w-7xl mx-auto mb-10">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-sm font-semibold text-slate-500 uppercase tracking-wider mr-2 w-full md:w-auto">
              Layanan:
            </span>
            ...
            return (
              <div key={`${loc.serviceType}-${loc.id}`} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col">
                ...
                <div className="h-56 w-full bg-slate-200 relative overflow-hidden">
                  ...
                  />
                  {/* Badge Kota */}
                  <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-slate-700">
                    {loc.city}
                  </div>
                </div>
        ...
        {filteredData.length === 0 && (
          <div className="text-center py-24 bg-white rounded-2xl border border-slate-200 shadow-sm">
            ...
            <p className="text-slate-500">
              Maaf, kami tidak menemukan lokasi yang sesuai dengan filter atau pencarian Anda. 
              Coba gunakan kata kunci lain atau hapus filter.
            </p>
            <button 
              onClick={() => { setSearchQuery(""); setActiveService("Semua"); setActiveCity("Semua"); }}
